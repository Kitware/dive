...
Zip import also accepts zip archive files that were generated by the ==Download== :material-arrow-right: ==Everything== export button (including full **stereo** and **multicam** dataset exports, which include `multiCam.json` and per-camera subfolders).

### Uploading multicamera and stereo data

DIVE Web supports importing **stereo** (2 cameras + calibration) and **multicam** (2 or 3 cameras) datasets from the same upload dialog used for single-camera data.
