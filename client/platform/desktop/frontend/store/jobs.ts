...
const cancelJobExitCode = 143; // SIGTERM
...
export function updateHistory(args: DesktopJobUpdate) {
  ...
  if (args.cancelledJob === true) {
    ...
    existing.job.exitCode = cancelJobExitCode; // SIGTERM
  }
...
function init() {
  ...
  ipcRenderer.on('cancel-job', (event, args: DesktopJob) => {
    updateHistory({
      ...args, body: ['Job cancelled by user'], exitCode: cancelJobExitCode, endTime: new Date(), cancelledJob: true,
    });
  });
}
