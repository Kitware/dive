...
describe('native.stereoCollectImport', () => {
  ...
  it('produces args that import as a stereo dataset', async () => {
    ...
    expect(imported.jsonMeta.subType).toBe('stereo');
    expect(Object.keys(imported.jsonMeta.multiCam?.cameras ?? {})).toEqual(['left', 'right']);
    expect(imported.jsonMeta.multiCam?.defaultDisplay).toBe('left');
    expect(imported.jsonMeta.multiCam?.cameras.left.originalBasePath)
      .toBe('/survey/fl01/left');
  });
  ...
  it('imports a root level video pair as a stereo dataset', async () => {
    ...
    const imported = await beginMultiCamImport(importArgs);
    expect(imported.jsonMeta.subType).toBe('stereo');
    expect(imported.jsonMeta.multiCam?.cameras.left.originalVideoFile)
      .toBe('dive01_left.mp4');
    expect(imported.jsonMeta.multiCam?.cameras.right.originalVideoFile)
