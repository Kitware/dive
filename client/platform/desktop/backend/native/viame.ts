...
async function runPipeline(
  ...
  if (meta.multiCam && stereoOrMultiCam) {
    ...
    const multiCamOrder = isMultiCamPipeline
      ...
      : undefined;
    // A kwiver .conf calibration (ifremer stereo rigs) is also a pipeline
    // config, and its pipes read every camera's tracks directly.
    const isConfCalibration = !!meta.multiCam.calibration?.toLowerCase().endsWith('.conf');
    const { argFilePair, outFiles } = await writeMultiCamStereoPipelineArgs(
      jobWorkDir,
      meta,
      settings,
      requiresInput || isConfCalibration,
      false,
      multiCamOrder,
      {
        imagePairs,
    ...
    if (meta.multiCam.calibration) {
      ...
      if (isConfCalibration) {
        command.push(`-c "${meta.multiCam.calibration}"`);
        const cameraNames = (multiCamOrder ?? orderedMultiCamCameraNames(meta.multiCam))
          .filter((name) => meta.multiCam && name in meta.multiCam.cameras);
        cameraNames.forEach((name, i) => {
          command.push(`-s cameras_calibration:tracks_${name}="${argFilePair[`detection_reader${i + 1}:file_name`]}"`);
        });
      }
    }
