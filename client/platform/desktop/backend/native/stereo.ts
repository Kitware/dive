 ...
 *
 * Manages a persistent Python subprocess that keeps the Foundation Stereo model loaded
 * for fast interactive stereo annotation. When enabled, the service proactively computes
 * disparity maps when the user navigates to new frames, so line transfers are instant.
 */
...
export class StereoServiceManager extends EventEmitter {
   ...
   * This spawns the Python process and loads the Foundation Stereo model.
   */
  ...
  private async _doInitialize(settings: Settings, calibration?: StereoCalibration): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      // Build the command to run the stereo service
      const command = [
        `. "${viameSetup}"`,
        '&&',
        'python -m viame.pytorch.foundation_stereo_interactive',
