...
export default class TrackManager {
  ...
  registerTrack(trackId: AnnotationId, trackActor: vtkActor, frameDetections: Array<[FrameNumber, vtkActor]>) {
    ...
    frameDetections.forEach(([frameNumber, actor]) => {
      ...
      if (this.framesMap.has(frameNumber)) {
        this.framesMap.get(frameNumber)!.push(actor);
      } else {
