...
import TrackManager, { FrameNumber } from './TrackManager';
import useTrackDrawer from './useTrackDrawer';
import { injectAggregateController } from '../annotators/useMediaController';

export interface IViewUtils {
  rerender: () => void;
  createTrackActors: (trackColor: [number, number, number], features: Array<{x: number; y: number; z: number; frameNumber: number}>) => { trackActor: vtkActor; frameDetections: Array<[FrameNumber, vtkActor]> };
}
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const viewUtils: IViewUtils = {
      rerender: () => undefined,
      createTrackActors(trackColor: [number, number, number], features: Array<{x: number; y: number; z: number; frameNumber: number}>) {
    ...
    const { onSelectedTrackChange, onFrameChange, initializeTracks } = useTrackDrawer(trackManager, onlyShowSelectedTrack, viewUtils);
