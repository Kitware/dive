...
import { getTrackColor } from './trackUtils';
import { IViewUtils, IFeature } from './TrackViewer.vue';
...
export default function useTrackDrawer(
   ...
   */
  const emphasizeTrack = function emphasizeTrack(trackId: AnnotationId, trackActor?: vtkActor) {
    if (!trackActor) {
      ({ trackActor } = trackManager.getTrack(trackId)!);
    }
  ...
  const deEmphasizeTrack = function deEmphasizeTrack(trackId: AnnotationId, trackActor?: vtkActor) {
    ...
    const trackWithContext = filteredTracksRef.filteredAnnotations.value.find((track) => track.annotation.id === trackId);

    // If we do not find the track anymore in the list, this is because the track has been filtered out in the meantime
  ...
  const hideTrack = function hideTrack(trackId: AnnotationId, trackMap?: TrackTracker) {
    if (!trackMap) {
      trackMap = trackManager.getTrack(trackId)!;
    }
    trackMap.trackActor.setVisibility(false);
    for (const actor of trackMap.detectionsMap.values()) {
  ...
  const initializeTrack = function initializeTrack(trackWithContext: TrackWithContext) {
    ...
    const features: IFeature[] = annotation.features
      ...
      .filter((feature) => feature !== undefined) as IFeature[];
  ...
  const onOnlyShowSelectedTrackChange = function onOnlyShowSelectedTrackChange(newOnlyShowSelectedTrack: boolean) {
  ...
  const onFilteredAnnotationsChange = function onFilteredAnnotationsChange(annotations: AnnotationWithContext<Track>[]) {
    ...
    annotations.forEach((trackWithContext) => {
      ...
      if (!trackManager.hasTrack(trackId)) {
        initializeTrack(trackWithContext);
      } else {
        // The track exists, so just show it
        const trackTracker = trackManager.getTrack(trackId)!; // cannot be undefined
