...
export interface GeometryCompleteEvent {
  type: string;
  bounds?: RectBounds;
  frameNumber: number;
}
...
export default function useAnnotationClickHandling(options: {
  ...
  lineLayer: LineLayer;
  refreshLayers: () => void;
  onGeometryComplete?: (event: GeometryCompleteEvent) => void;
}) {
  const {
    camera,
    handler,
  ...
  function wireHandlers() {
    ...
    editAnnotationLayer.bus.$on('update:geojson', (
      ...
      // Captured before the handler may auto-advance the frame.
      const frameNumber = frameNumberRef.value;
      ...
      if (geometryCompleteEvent) {
        ...
        refreshLayers();
        onGeometryComplete?.({ type, bounds: mapped.bounds, frameNumber });
      }
    });
