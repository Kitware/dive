...
import type { GeometryCompleteEvent } from 'vue-media-annotator/components/layerManager/useAnnotationClickHandling';
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    function onTrackAdded(trackId: AnnotationId) {
      ...
      const frame = aggregateController.value.frame.value;
      const trackType = clientSettings.trackSettings.newTrackSettings.type;
      multiCamList.value.filter((camera) => camera !== selectedCamera.value)
        .forEach((camera) => {
          const trackStore = cameraStore.camMap.value.get(camera)?.trackStore;
          trackStore?.add(frame, trackType, undefined, trackId);
        });
    }

    function onGeometryAdded({ type, bounds, frameNumber }: GeometryCompleteEvent) {
      ...
      cameraStore.getTrackAll(trackId).forEach((track) => {
        ...
        handler.newTrackSettingsAfterLogic(track, {
    ...
    watch(attributes, (attrs) => {
      ...
      const trackViewerSettings = {
        component: TrackViewerSettings,
        description: 'Track Viewer Settings',
      };
      if (hasStereo3dAttributes.value) {
        context.register(trackViewerSettings);
      } else {
        context.unregister(trackViewerSettings);
        showTrack3dViewer.value = false;
      }
    });
  ...
  <v-main class="viewer">
    ...
    >
      ...
      <template #extension>
        ...
        <template v-if="hasStereo3dAttributes">
          <v-divider
            vertical
            class="mx-2"
          />
