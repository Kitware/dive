...
export default defineConfig({
  ...
  plugins: [
    ...
    electron({
      main: {
        entry: 'platform/desktop/background.ts',
        vite: {
          build: {
            sourcemap: true,
      ...
      renderer: {
        entry: 'platform/desktop/main.ts', // Use a different entry point for Electron renderer
        vite: {
          build: {
            sourcemap: true,
  ...
  define: {
    ...
    'process.env.VITE_BUILD_TARGET': 'electron',
  },
});
