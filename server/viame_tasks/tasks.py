...
def run_pipeline(self, path, pipeline):
    ...
    file_names = [file_name for file_name in os.listdir(path) if not (
        file_name.endswith('csv') or file_name.endswith('CSV') or os.path.isdir(os.path.join(path, file_name)))]
    print(file_names)
    ...
    if len(file_names) == 1:
        ...
        with tempfile.NamedTemporaryFile(suffix='.txt', delete=False) as temp2:
            temp2.writelines(((os.path.join(path, file_name)+'\n').encode()
...
def convert_video(self, path, folderId, token, auxiliaryFolderId):
    ...
    self.girder_client.addMetadataToItem(_file['itemId'], {'folderId': folderId, 'codec': 'h264'})
