"""Python parity check for frame-metadata source names.

``frame_metadata`` / ``frame-metadata`` with a ``.json``, ``.csv``, or ``.txt`` extension is the
...
harnesses assert the same accepted/rejected truth table so the two mirrors stay aligned.
"""
...
SOURCE_NAMES_FIXTURE = Path(__file__).parents[2] / 'testutils' / 'framemetadata.spec.json'
